...
do
  local systemChat = util.systemChat
  ...
  function A:ePrint(...)
    if A:isEnabled() then
      local format_string = (select(1, ...))
      local addon_prefix = format("%s: ", color_heading(A.name))
      systemChat(format("%s%s", addon_prefix, color_normal(format_string)),
        select(2, ...))
    end
  end
end
...
do  
  local format, join, split, match, tolower = string.format, strjoin, strsplit, 
